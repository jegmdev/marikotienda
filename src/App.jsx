...
function App() {
  ...
  const [perfiles, setPerfiles] = useState([]); // Nueva tabla de usuarios
  ...
  const registrarVenta = async (clienteNombre, producto, cantidad = 1, fechaManual = null) => {
    ...
    // Validación dinámica contra la tabla perfiles
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <main className="container mx-auto p-4 max-w-5xl">
        {view === 'catalogo' ? (
          <VistaCatalogo 
            productos={productos} 
            clientes={perfiles.map(u => u.nombre)} 
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mb-8 mt-4 flex flex-col items-center gap-4 text-center">
        <select 
          className="w-full max-w-LG bg-slate-50 border-2 border-slate-100 p-4 rounded-2xl font-bold text-lg text-center outline-none focus:border-indigo-300 transition-all appearance-none cursor-pointer" 
          value={user} 
        ...
        {user && (
          <div className="bg-indigo-50 text-indigo-600 px-6 py-2 rounded-full font-black text-sm uppercase italic animate-bounce">
            💰 Tu deuda actual: {fM(deudaPersonal)}
          </div>
        )}
      </div>
      ...
      {sinStock.length > 0 && (
        <>
          <div className="relative py-8">
            ...
            <div className="relative flex justify-center"><span className="bg-slate-50 px-4 text-xs font-black text-indigo-300 uppercase tracking-widest">Pronto Volveremos</span></div>
          </div>
...
function CardProducto({ producto: p, user, registrarVenta }) {
  ...
  return (
    <div className="bg-white p-2 rounded-[2rem] shadow-sm hover:shadow-2xl transition-all border border-slate-50 relative overflow-hidden group">
      <div className={`absolute top-4 right-4 px-3 py-1 rounded-full font-black text-[10px] shadow-sm z-10 ${p.stock > 5 ? 'bg-white text-slate-400' : 'bg-orange-500 text-white animate-pulse'}`}>
        {p.stock} DISP.
      </div>
      ...
      <div className="p-4 text-center">
        ...
        <div className="flex items-center justify-center gap-4 mb-4 bg-slate-50 rounded-xl p-1">
          <button onClick={() => setCant(Math.max(1, cant - 1))} className="w-8 h-8 flex items-center justify-center bg-white rounded-lg shadow-sm font-bold text-indigo-600 hover:bg-indigo-100 transition-colors">-</button>
          <span className="font-black text-slate-700 w-4">{cant}</span>
          <button onClick={() => setCant(Math.min(p.stock, cant + 1))} className="w-8 h-8 flex items-center justify-center bg-white rounded-lg shadow-sm font-bold text-indigo-600 hover:bg-indigo-100 transition-colors">+</button>
        </div>

        <button onClick={handleComprar} className="w-full py-4 rounded-2xl text-white font-black text-xs transition-all bg-emerald-500 hover:bg-emerald-600 shadow-lg shadow-emerald-100 active:scale-95 uppercase tracking-tighter">
...
function VistaAdmin({ ventas, productos, perfiles, registrarVenta, refresh }) {
  ...
  const guardarPerfil = async (e) => {
    e.preventDefault();
    if (!userForm.nombre || !userForm.pin) return;
    const { error } = await supabase.from('perfiles').insert([userForm]);
    if (error) alert("Error: El nombre ya existe o hay un problema de conexión.");
    else { setUserForm({ nombre: '', pin: '' }); refresh(); }
  };
  ...
  return (
    <div className="space-y-8 pb-20 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {/* SECCIÓN 1: GESTIÓN DE USUARIOS (NUEVA) */}
      <div className="bg-indigo-50 p-8 rounded-[2.5rem] border-2 border-indigo-100 shadow-sm">
        <h2 className="text-xl font-black uppercase text-indigo-700 mb-6 italic tracking-tight">👥 Gestión de Usuarios y PINs</h2>
        <form onSubmit={guardarPerfil} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <input placeholder="PIN de 4 dígitos" className="p-4 rounded-2xl bg-white font-bold outline-none border-2 border-transparent focus:border-indigo-300" value={userForm.pin} onChange={e => setUserForm({...userForm, pin: e.target.value})} required maxLength={4} />
          <button className="bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-800 transition-all uppercase text-xs shadow-lg">Registrar Usuario</button>
        </form>
          ...
          {perfiles.map(u => (
            <div key={u.id} className="bg-white p-4 rounded-2xl shadow-sm flex justify-between items-center border border-indigo-100">
              <div>
                ...
                <p className="text-[10px] font-mono text-indigo-500">PIN: {u.pin}</p>
              </div>
              <button onClick={async () => { if(confirm(`¿Eliminar a ${u.nombre}?`)) { await supabase.from('perfiles').delete().eq('id', u.id); refresh(); } }} className="text-red-300 hover:text-red-600 transition-colors">✕</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
        ...
        <div className="bg-slate-900 p-8 rounded-[2rem] text-white mb-6 flex justify-between items-center shadow-xl">
          <div>
          ...
          {filtroNombre && totalF > 0 && !filtroProducto && (
            <button onClick={async () => { if(confirm(`¿Confirmas pago total de ${fM(totalF)} para ${filtroNombre}?`)) { await supabase.from('ventas').update({ pagado: true }).in('id', ventasFiltradas.map(v=>v.id)); refresh(); } }} className="bg-emerald-500 p-4 rounded-2xl font-black text-xs hover:bg-emerald-400 uppercase">Liquidar Deuda ✅</button>
          )}
        </div>
              ...
              {ventasFiltradas.map(v => (
                <tr key={v.id} className="bg-slate-50 hover:bg-indigo-50 transition-colors group">
                  ...
                  <td className="py-4 rounded-r-2xl text-center">
                    <button onClick={async () => { if(confirm("¿Eliminar registro?")) { await supabase.from('ventas').delete().eq('id', v.id); refresh(); } }} className="text-red-300 hover:text-red-500 px-3 opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-indigo-600 p-8 rounded-[2.5rem] shadow-lg shadow-indigo-100">
        <h2 className="text-xl font-black mb-4 uppercase text-white italic">🛒 Carga Manual de Venta</h2>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <select className="p-4 rounded-2xl bg-white font-bold text-sm outline-none" value={manualUser} onChange={e => setManualUser(e.target.value)}>
            <option value="">¿A quién?</option>
            {perfiles.map(u => <option key={u.id} value={u.nombre}>{u.nombre}</option>)}
          </select>
          <select className="p-4 rounded-2xl bg-white font-bold text-sm outline-none" value={manualProdId} onChange={e => setManualProdId(e.target.value)}>
            <option value="">¿Qué?</option>
            {productos.map(p => <option key={p.id} value={p.id}>{p.nombre} ({p.stock})</option>)}
          </select>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
        ...
        <form onSubmit={guardarProducto} className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-12">
          ...
          <button className="bg-indigo-600 p-4 rounded-2xl font-black text-white col-span-full hover:bg-indigo-700 shadow-lg shadow-indigo-100 uppercase tracking-widest transition-all">
            {form.id ? 'Actualizar Producto' : 'Guardar en Menú'}
          </button>
        </form>

        <div className="border-t border-slate-100 pt-8">
          <input placeholder="🔍 Buscar en stock..." className="w-full bg-slate-50 p-4 rounded-2xl mb-6 outline-none border-2 border-slate-100 focus:border-indigo-200 font-bold" onChange={e => setBusquedaInventario(e.target.value)} />
              ...
              <div key={p.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${p.stock <= 0 ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-50 shadow-sm'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-[10px] font-bold uppercase ${p.stock <= 0 ? 'text-red-400' : 'text-slate-400'}`}>Stock: {p.stock}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => {setForm(p); window.scrollTo({top: 1000, behavior:'smooth'})}} className="bg-indigo-50 p-2 px-4 rounded-xl text-indigo-600 font-black text-[10px] hover:bg-indigo-600 hover:text-white transition-all">EDITAR</button>
                  <button onClick={async () => { if(confirm("¿Eliminar producto?")) { await supabase.from('productos').delete().eq('id', p.id); refresh(); } }} className="bg-red-50 p-2 px-3 rounded-xl text-red-400 font-bold text-[10px] hover:bg-red-500 hover:text-white transition-all">✕</button>
                </div>
              </div>
            ))}
          </div>
